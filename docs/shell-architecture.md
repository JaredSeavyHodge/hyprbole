...
- layer-shell surfaces for bar, dock, launcher, quick settings, OSD, notifications, lock/session overlays, and desktop widgets
...
This mode is per subsystem, not global. Keybindings can be runtime-owned while monitors and appearance remain user-config-owned. The current implementation can reconcile runtime keybindings, has partial runtime action/status adapters for audio and brightness, performs observe-only reconciliation for monitors and workspaces with structured severity/results, and persists notification/DND and theme settings through Hyprbole-owned settings. Power/session ownership remains reserved for later slices.

Hyprbole-owned shell UI settings are separate from Hyprland ownership. The current prototype persists `ui.bar` settings in `$XDG_CONFIG_HOME/hyprbole/settings.json` for bar enablement, monitor selector, top/bottom edge, height, margin, padding, bitmap font size, rounded section radius, opacity, and widget visibility/order. It also persists `ui.osd` settings for OSD enablement, top/bottom edge, width, height, margin, visible timeout, bitmap font size, rounded card radius, and opacity. These settings drive native layer-shell surfaces directly and do not rewrite user Hyprland config. The bar currently applies edge, height, margin, padding, font size, radius, opacity, ordered system widgets, and tokenized colors from daemon snapshots. The OSD applies its persisted surface/style settings and tokenized colors from daemon snapshots, and it defers action display until daemon `ui.osd` settings have loaded. Monitor targeting resolves focused, primary, and named targets by matching daemon monitor snapshots to Wayland output names when available; unresolved targets fall back safely and are logged. The same settings file now stores `theme.mode`, and the daemon exposes effective built-in theme tokens in state snapshots for shell surfaces.
...
The first implemented runtime-owned keybinding reconciliation registers Super mouse drag/resize binds and `Super+Alt+Space` for `hbctl launcher` through Hyprland IPC. If keybindings are changed to `respect_user_config`, the daemon releases those runtime binds and then skips future reconciliation.
...
- settings panels
- launcher providers, including app indexes and generic stdin/menu lists
...
The preview and control surfaces are intentionally normal test windows. The bar is a single-instance layer-shell smoke surface, quick settings is a single-instance layer-shell popup, the OSD is a single-instance lightweight transient layer-shell action feedback surface, and the default launcher is a GTK4 plus gtk4-layer-shell surface. `hbctl bar --toggle|--restart` controls the bar instance, `hbctl osd --foreground|--toggle|--restart` controls the OSD instance, and `hbctl quick --toggle|--restart` opens, dismisses, or replaces the quick popup using runtime identity/dismiss files. `hbctl surfaces [quick|bar|osd|launcher] [--plain]` reports local runtime status for quick, bar, OSD, and launcher, including PID identity, pidfile path, pidfile health, log path, and log health. Pidfile state is reported as `missing`, `ok`, `stale`, `invalid`, or `unreadable`; only parseable stale pidfiles are cleanup candidates. Log state is reported as `missing`, `unreadable`, `empty`, or `ok`. `hbctl surfaces start|stop|restart quick|bar|osd|launcher|all` is a unified lifecycle wrapper over the same guarded launch/stop paths, `hbctl surfaces doctor [quick|bar|osd|launcher|all] [--json|--commands] [--unhealthy-only] [--actionable-only] [--zero-ok] [--warnings-only|--errors-only]` is a read-only diagnostic helper with severity, issue labels, recommendations, structured safe follow-up commands, and running-surface log warnings, `hbctl surfaces check quick|bar|osd|launcher|all running|stopped|healthy|pidfile-ok|pidfile-missing|pidfile-stale|pidfile-invalid|pidfile-unreadable [--quiet] [--json]` is an immediate read-only assertion helper, `hbctl surfaces wait quick|bar|osd|launcher|all running|stopped|healthy|pidfile-ok|pidfile-missing|pidfile-stale|pidfile-invalid|pidfile-unreadable [--timeout-ms N] [--interval-ms N] [--quiet] [--json]` is a read-only wait helper over the same predicates with 3000ms timeout and 50ms interval defaults, and `hbctl surfaces clean quick|bar|osd|launcher|all [--dry-run]` removes only parseable stale pidfiles while preserving live, missing, invalid, or unreadable pidfiles. Check and wait JSON reports include `target`, `expected`, `passed`, and matching `surfaces`; wait also includes `elapsed_ms`. Doctor JSON reports include health, issue count, filter state, per-surface severity, issue labels, recommendations, optional `recommended_command`, log state, and embedded surface status. Quick/bar/OSD/launcher instance files store `pid:start_time` identities and are validated against `/proc`; stale files are recovered and pidfile-less old prototype processes are detected as a migration guard. The older egui quick settings window remains available with `hbctl quick --dev` for development comparisons. This keeps the current renderer useful for validating layout, visual language, interactive shell actions, and module shape quickly; the long-term runtime target remains a shell-specific Rust Wayland renderer.

The current prototype can refresh shell state, focus Hyprland workspaces, adjust output volume, toggle output mute, adjust brightness, toggle notification DND, switch runtime layout/monitor mode, toggle quick settings from the bar, show daemon action feedback in the OSD, edit persisted bar/OSD settings and widget order, edit persisted theme mode, and request session lock through daemon IPC. The control center now has dedicated Bar and OSD panes for those persisted shell surface settings. The bar renders daemon-backed workspace, active-window, audio, brightness, notification/DND, layout, clock, action, and daemon/reconcile status text according to persisted `ui.bar.widgets`, applies configured margin/padding, and bar/quick/OSD consume daemon-provided color tokens for core text/background colors. The OSD follows daemon events, refreshes snapshots/status on a background worker, defers action display until daemon `ui.osd` settings load, applies persisted `ui.osd` surface settings, auto-hides when idle, and renders recent action status plus current audio/brightness summaries from snapshots. Bar hit regions route visible workspace, status, and system interactions explicitly, and rapid scroll/toggle actions are lightly throttled client-side before daemon dispatch. Refreshes and actions run through background workers so surfaces remain responsive. Direct UI fallback paths are debug-only. The daemon handler path now has a mockable runtime seam for tests so state/settings/action/event behavior can be covered without a live Hyprland session. Notification history/DND state exists, but replacing a DBus notification daemon is future work.

The layer launcher writes `launcher.pid` while running; egui and GTK launcher prototype paths remain outside `hbctl surfaces` lifecycle control.

For `hbctl surfaces doctor`, missing logs are only treated as issues for running surfaces; unreadable or empty logs are reported whenever present. `--unhealthy-only` filters healthy rows, `--actionable-only` filters rows with safe follow-up commands, `--commands` prints only displayed safe recommended commands, `--zero-ok` preserves output while returning success, and `--warnings-only` or `--errors-only` filter output by severity; severity filters are mutually exclusive, `issues`, `warnings`, `errors`, and `actionable` remain pre-filter, `displayed` reports the post-filter row count in JSON, and plain output starts with the same expanded summary counts plus a no-match message for empty filtered results.
